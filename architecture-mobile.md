# 摄影师的 AI 接单跟单助手 — 移动端扩张架构设计 + 任务分解（React Native App + 微信小程序 + Web 适配 + 后端最小扩展）

> 文档：架构设计 v4.0（移动端扩张版 / 聚焦 App + 小程序）
> 作者：架构师 高见远（Gao）
> 日期：2026-07-24
> 范围：**三端统一（Web 已存在 / RN(Expo) App(安卓+iOS) / Taro 微信小程序）+ 后端最小扩展（微信登录 / 推送 / 对象存储直传）**；**不重写**既有 Web 与后端。
> 基线：`architecture.md`（v1.0 Web MVP 阶段1）、`architecture-phase2.md`（v2.0）、`architecture-phase3.md`（v3.0 订阅/团队/看板/校准，小程序独立立项复用本设计落地的 REST API）。
> 配套图：类图 / 时序图以 Mermaid 内嵌于本文 §3 / §4（与仓库既有 `architecture-*.md` 风格一致）。

---

## 0. 设计总纲（最小变更 + 最大复用）

- **后端零重写**：复用现有 Spring Boot 3.2 / JPA / PostgreSQL / Flyway / `Result{code,data,message}` / `CurrentUser.getStudioId()` 多租户隔离。仅在 `auth` 扩微信登录、`push` / `storage` 两个新 module；AI 报价/脚本生成沿用既有 `LlmClient`（OpenAI 兼容，默认 DeepSeek `deepseek-chat`，`DEEPSEEK_API_KEY` 环境变量，模型可配 `deepseek.model`）。
- **前端最大复用**：三端**全 React 技术栈**。把 Web 现有 `types/models.ts`、`api/client.ts` 核心、`api/*.ts` 端点、`TanStack Query` hooks、`Zustand` store 抽成 **共享包 `@photogai/shared`**；Web / RN / Taro **共用同一份类型、API 客户端、领域逻辑、Query hooks、状态管理**，**仅 UI 组件层因平台差异各自实现**（MUI / RN 原生组件 / Taro 组件）。
- **仓库形态**：**pnpm workspace 单仓（monorepo）**。现有 `web` 与 `backend` 是两个独立仓库 → 迁移进同一 git 仓（backend 作兄弟目录，不进 pnpm workspace 以免 npm 工具链冲突）。若暂不便合仓，退路是 `@photogai/shared` 发私有 npm 包（见 §1.1）。**本设计按 monorepo 落地**。
...
### 1.1 仓库形态：monorepo（推荐）还是独立仓库

**结论：采用 monorepo（pnpm workspace + 可选 Turborepo 缓存）。**

现有 `web` 与 `backend` 是独立仓库，迁移方案如下：

| 方案 | 做法 | 取舍 |
|---|---|---|
| **✅ monorepo（推荐）** | 新建 git 根仓库；`web` 迁入 `apps/web/`；新建 `apps/mobile/`、`apps/miniprogram/`、`packages/shared/`；`backend` 作为 `photographer-ai-backend/` 兄弟目录（**不加入 pnpm-workspace.yaml**，避免 Java/Maven 污染 npm 工具链）；CI 中 backend 单独一个 job 用 Maven 构建 | 类型/API client 单一真源；`workspace:*` 即时感知共享包变更；Turborepo 缓存加速。一次性迁移成本，长期收益最大 |
| 独立仓库 + 私有 npm 包 | `@photogai/shared` 发成私有包（Verdaccio / npm scope），三端各自安装 | 不需动现有 web/backend 仓；但每次共享包变更要发版，三端版本易漂移，协作摩擦大 |

> 若团队坚持保留 web/backend 两个独立仓、不愿合仓：**退路是把 `@photogai/shared` 做成私有 npm 包**，App 与小程序从它拉取类型/API client/逻辑，web 也引用同一包（web 可保留在独立仓但 `npm i @photogai/shared`）。本设计正文按 monorepo 写，迁移步骤见 §1.4。
...
├── apps/
│   ├── web/                              # 既有 web 仓库迁入（Vite+React+MUI+Tailwind）
│   │   ├── src/
...
│   ├── mobile/                          # 新建：React Native + Expo（安卓+iOS）
...
**共享包导出契约**：`@photogai/shared` 的 `package.json` 用 `exports` 字段按子路径导出（`@photogai/shared/types`、`/api`、`/hooks`、`/store`、`/domain`、`/http`），并声明 `peerDependencies: react, @tanstack/react-query, zustand, axios`，避免三端重复装包。

### 1.4 Web 仓库迁入 monorepo + 抽共享包（不重写）
...
### 2.1 后端新增/修改文件（相对 `photographer-ai-backend/`）
...
| `modules/auth/entity/UserWechat.java` | **+** | 微信绑定实体（含 union_id / openid / session_key / app_type） |
...
| `modules/push/entity/DeviceToken.java` | **+** | 设备令牌实体（platform=IOS/ANDROID/MP） |
...
### 2.2 共享包新增/迁出文件（相对 `packages/shared/`）
...
classDiagram
    ...
    class UserWechat {
        ...
        +Long userId
        +Long studioId
        +String appType    %% WEB|APP|MP
        +String openid
        +String unionId    %% 三端同一 studio 的纽带
        +String sessionKey %% 仅 MP
    }
    class DeviceToken {
        +Long id
        +Long userId
        +Long studioId
        +String platform   %% IOS|ANDROID|MP
        +String token      %% Expo/FCM/APNs/MP openid
        +String appVersion
    ...
    class UploadFile {
        ...
        +Long studioId
        +Long ownerUserId
        +String bizType    %% ORDER_SAMPLE|CUSTOMER_AVATAR|STUDIO_LOGO
        +String fileKey
        +String url
    }
    ...
    Studio "1" --> "*" UserWechat : 隔离
    Studio "1" --> "*" DeviceToken : 隔离
    Studio "1" --> "*" UploadFile : 隔离
    User "1" --> "*" UserWechat : 绑定多端
    User "1" --> "*" DeviceToken : 多设备
    User "1" --> "*" Order : createdBy
    Customer "1" --> "*" Order : 历史订单
    Order "0..1" --> "assignedTo" User : 团队分配(可空)
```

> 扩展说明：核心实体 `User/Studio/Order/Quote/Subscription` 中，`User` 新增 `avatarUrl`（头像，支撑 App/小程序头像展示）；`Quote` 为 AI 报价出参（`QuoteResponse`），无独立持久化表（按请求实时生成）；`Subscription` 沿用阶段3 实体（PRO/TEAM 有效期真源）。新增 `user_wechat`（微信 UnionID/openid 绑定，三端打通纽带）、`device_token`（iOS/Android/MP 推送令牌）、`upload_file`（样片/头像对象存储直传记录）。

### 3.2 后端迁移 SQL（`V4__multiterminal.sql`）

```sql
-- 移动端扩张：微信绑定 + 设备令牌 + 对象存储直传（2026-07-24）
...
-- 4) users 扩 avatar_url（头像，支撑移动端展示）；VARCHAR 直接加列
...
## 5. 任务列表（按模块，含依赖、顺序、可并行）

> 建议落地顺序：**先后端扩展（T1/T2/T3 可并行）+ 共享包（T4）**，再三端并行（T5 Web 适配 / T6 RN App / T7 Taro 小程序），最后 CI（T8）。后端不依赖 monorepo，可最早启动。

| ID | 模块 | 任务 | 关键文件 | 依赖 | 优先级 | 可并行 |
|---|---|---|---|---|---|---|
| **T0** | 仓库 | Monorepo 脚手架 + 共享包骨架 | 根 `package.json`/`pnpm-workspace.yaml`/`turbo.json`/`tsconfig.base.json`；`packages/shared`(`package.json`/`tsconfig`/`src/index.ts` 空模块)；`apps/web` 迁入 | 无 | P0 | — |
| **T1** | 后端·认证 | 后端微信登录（统一账号） | `WechatConfig`+`WechatService`+`UserWechat`(实体/Repo)+`AuthController`(+`/wechat/login`/`/wechat/bind`)+3 DTO+`ErrorCode`(+2)+`V4__multiterminal.sql`(user_wechat)+`users.avatar_url` | 阶段3 基线 | P0 | ✅ 与 T2/T3 并行 |
| **T2** | 后端·推送 | 后端推送（设备令牌 + 扇出） | `DeviceToken`(实体/Repo)+`PushService`(Expo+微信订阅)+`PushController`(+`/push/device`/`/test`)+`ErrorCode`(+1)+`V4`(device_token) | T1(user_wechat 供 MP openid) | P1 | ✅ 与 T3 并行（IOS/ANDROID 分支独立于 T1） |
| **T3** | 后端·存储 | 后端对象存储直传（预签名） | `UploadFile`(实体/Repo)+`StorageService`(presign/confirm/receive)+`StorageController`(+`/presign`/`/confirm`/`/upload`)+`V4`(upload_file) | 无 | P1 | ✅ 与 T1/T2 并行 |
| **T4** | 共享包 | 共享包业务内容 | 迁出 `types/models.ts`→shared；抽 `http/HttpClient.ts`；`api/*`(含 push/storage)；`domain/*`+`constants.ts`；`hooks/*`；`store/*`(StorageAdapter)；新增 `types/multiterminal.ts` | T0；接口契约按本文档先写，后端落地后对齐 | P0 | ✅ 与 T5 部分并行 |
| **T5** | Web 适配 | Web 适配（迁入 + 消费共享 + 微信扫码登录） | `apps/web` 删自有 `api/*.ts`/`types/models.ts`，改引 `@photogai/shared`；`lib/http.ts`(web 存储+401/402/403 UX)；`lib/storage.ts`；`LoginPage` 加微信扫码 | T4；T1(微信扫码后端) | P1 | ✅ 与 T6/T7 并行 |
| **T6** | RN App | React Native + Expo App | `apps/mobile` 脚手架(expo-router/nativewind)；接 `@photogai/shared`；`screens/*`(登录/订单/日历/AI报价/客户/订阅)；`lib/{http,storage,wechat,push}.ts`；expo-notifications + 微信开放平台登录 | T4；T1(微信登录)；T2(推送) | P1 | ✅ 与 T7 强并行 |
| **T7** | 小程序 | Taro 微信小程序 | `apps/miniprogram` 脚手架(taro weapp/taro-ui)；接 `@photogai/shared`(axios+axios-taro-adapter)；`pages/*`；`lib/{http,storage,wechat}.ts`(Taro.login + 订阅消息授权) | T4；T1(微信登录)；T2(订阅消息) | P1 | ✅ 与 T6 强并行 |
| **T8** | CI-CD | CI/CD 三端 + 后端 | GitHub Actions：backend(Maven test/build)、web/mobile/miniprogram(pnpm+ turbo；EAS Build / Taro build weapp) | T5/T6/T7 完成 | P2 | 各端流水线独立 |
...
- **建议并行度**：后端 T1/T2/T3 与共享包 T4 可同期开工；T5/T6/T7 在 T4 就绪后**三端并行**（T6 与 T7 强并行）。是否三端同日发布由 PM 决定（小程序需微信审核、App 需商店审核）。
...
```
> 共享包本体**不引入 UI 库**；`zod`（可选，运行时校验 DTO）可按需加。
...
| **主题/视觉一致性（极简两主色）** | 主色 `#2D6CDF`（主操作/激活）、墨色 `#1A1A1A`（文字/重要）；背景白 `#FFFFFF`、浅灰分隔 `#F2F4F7`、圆角 10–12。三端复用 shared `constants` 中的色 token；RN 用 nativewind 映射，小程序用 taro-ui 主题变量。最多两主色，禁止引入第三主色。 |
...
| 3 | **图片上传方式** | 默认**对象存储预签名直传**（OSS/COS/S3），后端不落盘大文件；dev 用 `LOCAL` 后端接收兜底。需定存储商（默认阿里云 OSS，国内合规）并提供 Bucket/密钥。不阻塞（dev 可先 LOCAL）。 |
...
| 6 | **现有 web 仓库是否迁入 monorepo** | 默认**迁入** `apps/web/`（推荐，最大复用共享包）；若团队不便合仓，退路是把 `@photogai/shared` 发私有 npm 包、web 独立仓引用。**本设计按迁入落地**。 |
| 7 | **Web 端是否需要设备推送** | Web 暂不做浏览器推送（无 `device_token`），仅站内提醒；如需 Web 推送后续补 `platform=WEB` 走 Web Push API。默认不阻塞。 |
| 8 | **Web 微信扫码登录** | 需开放平台**网站应用** AppID（与 App 同一开放平台）。默认 Web 同时保留"账号密码 + 微信扫码"两种登录。 |

---

*移动端扩张架构结束 — 回传主理人齐活林评审，并交工程师按 T0 → T1/T2/T3（后端并行）→ T4（共享包）→ T5/T6/T7（三端并行）→ T8 实现。小程序独立立项复用本设计落地的 REST API（与阶段3-B 一致）。`architecture.md`（Web MVP 阶段1）保持不变。*
